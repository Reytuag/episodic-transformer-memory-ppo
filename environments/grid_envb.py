...
def test_recipes(items, recipes):
    ...
    reward = jnp.select([product == 0, product == 4], [0.05, 1.])
...
class Gridworld(VectorizedTask):
    ...
    def __init__(self,
        ...
        def rest_keep_recipe(key, recipes, grid_color, steps):
            next_key, key = random.split(key)
            posx, posy = (1, 1)
            agent = AgentState(posx=posx, posy=posy, inventory=0)
            grid = get_init_state_fn(key)
        ...
        def step_fn(state, action):
            ...
            steps = state.steps + 1
            r_done = jnp.logical_or(grid[:, :, -2].sum() > 0, steps > self.max_steps)
            ...
            state = jax.lax.cond(
                r_done,
                lambda x: rest_keep_recipe(key, state.permutation_recipe, grid_color=state.grid_color, steps=steps),
...
class Grid:
    def __init__(self,max_episode_steps=256):
        ...
        self.key=jax.random.PRNGKey(np.random.randint(42))
    ...
    def reset(self):
        ...
        obs=state.obs[0]
        r=state.reward[0]
        la=state.last_action[0]
        return jnp.concatenate([obs,r,la])

    def step(self, action):
        state, reward, done = self._env.step(self.state,action)
        self.state=state
        done=done[0]
        obs = state.obs[0]
        reward = state.reward[0]
        la = state.last_action[0]
        obs=jnp.concatenate([obs,reward,la])
